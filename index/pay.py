...
def get_ali_object():
    ...
    app_id = "2016101300676484"  #  APPID （沙箱应用）

    # 支付完成后，支付偷偷向这里地址发送一个post请求，识别公网IP,如果是 192.168.20.13局域网IP ,支付宝找不到，def page2() 接收不到这个请求
    notify_url = "http://127.0.0.1:8000/vue/page1_"

    # 支付完成后，跳转的地址。
    return_url = "http://127.0.0.1:8000/vue/page1_"
    app_private_key_path = "./keys/alipay_private_2048.txt" # 应用私钥
...
def PaGe(request,id):
    if request.method == "GET":
        ...
        Orders.objects.filter(code=code).update(status='已支付')
